...
{ quick; 
  ...
  freopen("output.txt", "w", stdout);
    ...
    {   
        ...
        {
            ll x; cin>>x;
            // PAIR.FIRST REPRESENTS ARRAY ELEMENT AND PAIR.SECOND REPRESENTS TOTAL SWAPS AFTER SORTING THE ARRAY
            a[i] = {x,0};
        }
        // APPLYING BUBBLE SORT TO SORT THE ARRAY IN DESCENDING ORDER
        for(ll i = 0; i < n-1; i++)
        {      
        ...
        // CHECK IF IT'S POSSIBLE TO KEEP THE ARRAY SAME BY SWAPING THE ELEMENTS HAVING EVEN NUMBER OF SWAPS
        loop(0,n-1,i)
        {
            if(a[i].f==a[i+1].f and a[i].se%2==0 and a[i+1].se%2==0)
            {
                a[i].se++;
                a[i+1].se++;
                swap(a[i], a[i+1]);
