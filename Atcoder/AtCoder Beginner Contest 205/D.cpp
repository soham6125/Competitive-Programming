...
{ quick; 
  ...
  freopen("output.txt", "w", stdout);
    ...
    while(tc--)
    {
        ...
        vll v(n,0), pref(n,0);
        loop(0,n,i) cin >> v[i];
        loop(1,n,i) pref[i] = v[i] - v[i-1] - 1;
        pref[0] = v[0] - 1;
        loop(1,n,i) pref[i] += pref[i-1];
        // for(auto it:pref) cout<<it<<" ";
        while(q--)
        {
            ll x; cin>>x;
            if(x<=pref[0]) cout<<x<<endl;
            else if(x>pref[n-1]) cout << v[n-1] + (x-pref[n-1]) << endl;
            else
            {
                ll ind = lb(all(pref),x) - pref.begin();
                if(pref[ind]!=x) ind--;
                ll ans = 0;
                if(pref[ind]==x) ans = v[ind]-1;
                else ans = v[ind] + (x - pref[ind]);
                cout << ans << endl;
            }
        }
    }
